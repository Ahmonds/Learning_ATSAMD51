...
#define levelingAggression 30             // 90  maps ≈ [ 0° - 45° ] to [ 50% - 100% ], 180 maps ≈ [ 0° - 80° ] to [ 50% - 100% ]
#define Kp 0.9
#define Ki 0.9
float levelingErr = 0;
...
void ControlledLevel(float now) {
   ...
   if (levelingDUT <= 1450 && levelingDUT >= 950) levelingDUT = 1199;      // (1199-950)/1199  = 20.767%
...
switch(mode2) {
   case 0:        // PI Controllers
      ...
      if (  !speedMode && speedyMode ) {
         speedy = (float) (millis() - speedyTime);
         speedyTime = millis();
